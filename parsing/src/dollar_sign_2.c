...
/*   Updated: 2022/11/12 16:12:52 by fakouyat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	v->var_ds = malloc(sizeof(char *) * (v->ct1[2] + 1));
	if (!v->var_ds)
	{
		free(v->var_ds);
		v->var_ds = NULL;
...
void	extract_ds_vars(t_prgm *v)
{
	...
	while (v->tmp[v->ct1[1]] != '\0' && (v->ct1[5] < v->ct1[2]))
	{
		if (v->tmp[v->ct1[1]] == '$')
		{
			v->tokens[v->ct1[0]].rf_d[v->ct1[5]] = v->ct1[1];
			v->ct1[1]++;
			v->ct1[3] = v->ct1[1];
			v->ct1[4] = 0;
			...
			v->var_ds[v->ct1[5]] = ft_substr(v->tmp, v->ct1[3], v->ct1[4]);
			v->ct1[5]++;
		}
		else
			v->ct1[1]++;
...
void	translate_var_helper(t_prgm *v, t_env *tmp_node, int flag)
{
	while (tmp_node != NULL)
	{
		if (ft_strcmp(v->var_ds[v->ct1[6]], tmp_node->key) == 0)
		{
			ft_str_replace(&v->tokens[v->ct1[0]].t_str, \
				ft_strjoin("$", v->var_ds[v->ct1[6]]), \
				ft_strdup(tmp_node->value), \
				v->tokens[v->ct1[0]].rf_d[v->ct1[6]]);
			flag = 1;
		}
	...
	else if (flag == 0 && *v->var_ds[v->ct1[6]] == '\0')
		ft_str_replace(&v->tokens[v->ct1[0]].t_str, "$", "$",
		v->tokens[v->ct1[0]].rf_d[v->ct1[6]]);
	else if (flag == 0)
		ft_str_replace(&v->tokens[v->ct1[0]].t_str, \
		ft_strjoin("$", v->var_ds[v->ct1[6]]), \
		ft_strdup(""), v->tokens[v->ct1[0]].rf_d[v->ct1[6]]);
}
...
void	translate_var(t_prgm *v)
{
	...
	while (v->ct1[6] < v->ct1[2])
	{
		...
		if (expand_ds(v->tokens[v->ct1[0]].t_str_og, \
			v->tokens[v->ct1[0]].rf_d[v->ct1[6]]) == 1)
