...
/*   Updated: 2022/11/05 21:48:32 by mochan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 		if ((vars->tokens[vars->i].in != IN_STD && \
// 			vars->tokens[vars->i].in == IN_HEREDOC) || \
// 			(vars->tokens[vars->i].in != IN_STD && \
// 			vars->tokens[vars->i].in == IN_REDIRECT))
...
// 		if (vars->tokens[vars->i].in != IN_STD && \
// 			vars->tokens[vars->i].out != OUT_STD)
// 			find_cmd_opt_arg_both_redirections(vars);
// 		else if (vars->tokens[vars->i].in != IN_STD && \
// 			vars->tokens[vars->i].out == OUT_STD)
