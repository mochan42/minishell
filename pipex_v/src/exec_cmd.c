...
void	ft_exec_cmd_1(t_prgm *vars, int *let_error)
{
	...
	else
	{
		if (ft_strncmp(vars->tokens[0].out, ">>", 2) == 0)
			vars->tokens[0].fd_args[1] = open(vars->tokens[0].outfile, O_RDWR | O_CREAT | O_APPEND, 0777);
		else if (ft_strncmp(vars->tokens[0].out, ">", 1) == 0)
			vars->tokens[0].fd_args[1] = open(vars->tokens[0].outfile, O_RDWR | O_CREAT | O_TRUNC, 0777);
...
void	ft_exec_cmd_last(t_prgm *vars, int *let_error)
{
	...
	if (ft_strncmp(vars->tokens[vars->pipe_ct].out, ">>", 2) == 0)
		vars->tokens[vars->pipe_ct].fd_args[1] = open(vars->tokens[vars->pipe_ct].outfile, O_RDWR | O_CREAT | O_APPEND, 0777);
	else if (ft_strncmp(vars->tokens[vars->pipe_ct].out, ">", 1) == 0)
		vars->tokens[vars->pipe_ct].fd_args[1] = open(vars->tokens[vars->pipe_ct].outfile, O_RDWR | O_CREAT | O_TRUNC, 0777);
