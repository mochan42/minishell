...
void	ft_infile_checking(t_prgm *vars, int *let_error)
{
	if (access(vars->tokens[vars->p.child].infile, F_OK | R_OK) != 0)
	{
		...
		ft_strcat(vars->p.error[vars->p.child],
			vars->tokens[vars->p.child].infile);
		*let_error = 1;
	}
	else
		vars->tokens[vars->p.child].fd_args[0] = open(
				vars->tokens[vars->p.child].infile, O_RDONLY, 0777);
}
...
void	ft_in_redirect(t_prgm *vars, int *let_error)
{
	...
	if (ft_strncmp(vars->tokens[vars->p.child].in, "<<", 2) == 0
		|| ft_strncmp(vars->tokens[vars->p.child].in, "<", 1) == 0)
	{
...
void	ft_redirect_out(t_prgm	*vars)
{
	...
	if (ft_strncmp(vars->tokens[vars->p.child].out, ">>", 2) == 0)
		vars->tokens[vars->p.child].fd_args[1] = open(
	...
	if (ft_strncmp(vars->tokens[vars->p.child].out, ">>", 2) == 0
		|| ft_strncmp(vars->tokens[vars->p.child].out, ">", 1) == 0)
	{
