...
void	ft_set_cmd_error_msg(t_prgm *vars, char **tmp, int cmd)
{
		...
		&& (vars->p.error[cmd][0] == 0) && vars->tok[cmd].built_in == 0)
	{
		...
		ft_strcat(vars->p.error[cmd], vars->tok[cmd].opts[0]);
		ft_exit_code(127, 1);
	}
}
...
void	ft_parse(t_prgm *vars, char **paths, int cmd)
{
	...
	while (paths[i])
	{
		if (vars->tok[cmd].opts[0][0] != '/')
			tmp[0] = ft_strjoin("/", vars->tok[cmd].opts[0]);
		else
			tmp[0] = ft_strjoin("../", vars->tok[cmd].opts[0]);
...
int	ft_assert_numeric(t_prgm *vars, int indice)
{
	...
	j = 0;
	while (vars->tok[vars->p.child].opts[indice][j])
	{
		if (is_numeric(
				vars->tok[vars->p.child].opts[indice][j]) == 1)
		{
			printf("numeric argument required : %s\n",
				vars->tok[vars->p.child].opts[indice]);
			ft_exit_code(255, 1);
			return (1);
		}
		j++;
